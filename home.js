   ...
   .then((data) =>{
     ...
     for(let i = 0; i < data.length; i++){
       ...
       .then((dataImg)=>{
         ...
         let imgDiv =`
           <div id="ThumbnailImages">
           <a href="${dataImg.video_link}" target="_blank">
             <img class="Image1" src="${dataImg.image_link}">
             </a>
           </div>
           `;
...
{/* <script type="text/javascript">
	var movesite=document.querySelector('select');
	movesite.onchange = function(){//select가 변경될때 이벤트 이름 = onchange
		if(this.value){ //값이 아무것도 없으면 false로 오니까 굳이 this.value == null이라고 적을 필요가 없음
			location.assign("https://www."+this.value); // assign :
		}
	}
</script> */}
